...
class S3ImageService(object):
    ...
    def _translate_uuid_to_id(self, context, image):
        ...
        #注意这里所用的拷贝方法不是deepcopy，所以虽然实现拷贝但是实际上二者共享同一数据，所以image_copy改变的id值，同样image元数据中同样更新，所以直接返回了image_copy给方法def _get_image(self, context, ec2_id)
    ...
    def show(self, context, image_id):
        ...
        ###主要完成了：调用/nova/image/glance.py----def show(self, context, image_id)方法，完成通过glance客户端获取image_uuid指定的镜像元数据，并转换为python可处理的数据格式返回
